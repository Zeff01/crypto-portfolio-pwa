	...
	async function login({ email, password }: typeof initialValues) {
		...
		try {
			...
			if (res.status === 200) {
				...
				save(data);
				//TODO: figure out why navigate does not work after signin
				navigate('/');
			}
		} catch (error) {
					...
					<Form className='flex flex-col gap-2 pt-4 pb-2'>
						<div className='flex flex-col h-[80px] justify-start'>
							...
							<Field
								type='text'
								name='email'
								className='border-b border-custom-border py-1 px-2 bg-transparent'
							/>
						...
						<p className='mt-10 text-sm dark:text-custom-text'>
							Forgot password?
							<Link
								to='/reset'
